    ...
    public static List<BookedTable> btParser(CSVParser csvRecords, String shopId) {
        ...
        try {
            for (CSVRecord record : csvRecords) {
             ...
             //   logger.info("setSubsidieEffect解析值过多", +Double.valueOf(record.get(21).toString()));
              //  logger.info("setSubsidieEffect解析值过多", +Double.valueOf(df.format(Double.valueOf(record.get(21).toString()))));
                bt.setCommissionEffect(Double.valueOf(record.get(21).toString()));
             //   logger.info("setCommissionEffect解析值过多", +Double.valueOf(record.get(21).toString()));
             //   logger.info("setCommissionEffect解析值过多", +Double.valueOf(df.format(Double.valueOf(record.get(21).toString()))));
